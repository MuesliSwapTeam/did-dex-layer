...
class DevelopmentEnvironment:
    ...
    def _check_system_requirements(self, config: Dict[str, Any]):
        ...
        if not self._version_meets_requirement(python_version, required_python):
            raise Exception(f"Python {requirements['python']} required, found {python_version.major}.{python_version.minor}")
    ...
    def _setup_nodejs_environment(self):
        ...
        # Setup frontend
        frontend_path = os.path.join(self.project_root, "src/auth_nft_minting_tool/frontend")
        if os.path.exists(frontend_path):
            self._setup_frontend_environment(frontend_path)
        
        # Setup server
        server_path = os.path.join(self.project_root, "src/auth_nft_minting_tool/server")
...
"""
    ...
    def _install_opshin(self):
        """Install OpShin."""
        try:
            subprocess.run([sys.executable, "-m", "pip", "install", "opshin"], check=True)

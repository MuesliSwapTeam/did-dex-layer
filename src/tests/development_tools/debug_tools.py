...
import pycardano
from pycardano import Transaction, TransactionBuilder, TransactionOutput, Value, MultiAsset, Asset
...
class OrderbookDebugger:
    ...
    def debug_order_creation(self, trader_name: str, order_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def debug_order_matching(self, orders: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            debug_info['success'] = True
            print(f"✅ Order matching debug completed: {debug_info['matches_found']} matches found")
    ...
    def debug_transaction_building(self, transaction_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for input_data in transaction_data.get('inputs', []):
                ...
                debug_info['steps'].append(f"Added input: {input_data.get('id', 'unknown')}")
            ...
            for output_data in transaction_data.get('outputs', []):
                ...
                debug_info['steps'].append(f"Added output: {output_data.get('id', 'unknown')}")
            ...
            for script_data in transaction_data.get('scripts', []):
                ...
                debug_info['steps'].append(f"Added script: {script_data.get('id', 'unknown')}")
    ...
    def create_order_parameters(self, trader: Dict, order_data: Dict[str, Any]) -> orderbook.OrderParams:
    ...
    def orders_compatible(self, order1: Dict[str, Any], order2: Dict[str, Any]) -> bool:
        ...
        return (order1.get('buy_token') == order2.get('sell_token') and
                order1.get('sell_token') == order2.get('buy_token'))
    
    def test_order_match(self, order1: Dict[str, Any], order2: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def validate_transaction_inputs(self, transaction_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def generate_debug_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'summary': {
                'successful_operations': len([log for log in self.debug_log if log.get('success', False)]),
                'failed_operations': len([log for log in self.debug_log if not log.get('success', False)]),
                'total_errors': sum(len(log.get('errors', [])) for log in self.debug_log)
            }
        }
...
def main():
    ...
    orders = [
        {'id': 'order1', 'buy_token': 'token_a', 'sell_token': 'token_b', 'amount': 100, 'price': 0.5},
        {'id': 'order2', 'buy_token': 'token_b', 'sell_token': 'token_a', 'amount': 50, 'price': 0.6}
    ]

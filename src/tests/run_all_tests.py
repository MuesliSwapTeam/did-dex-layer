...
class TestRunner:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "-m",
                    "pytest",
                    "tests/test_suite/test_orderbook_contracts.py",
                    "-v",
                ],
                cwd=project_root,
                capture_output=True,
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                passed_line = [
                    ...
                    if "passed" in l
                    and (
                        "failed" in l or "error" in l or "test session starts" not in l
                    )
                ]
                if passed_line:
    ...
    def print_summary(self, summary: Dict[str, Any]):
        ...
        for category, result in summary["test_results"].items():
            ...
            print(
                f"  {status_icon} {category}: {result['passed']} passed, {result['failed']} failed"
            )
...
def main():
    ...
    parser.add_argument(
        "--contracts-only", action="store_true", help="Run only contract tests"
    )
    ...
    if args.contracts_only:
        ...
        print(
            f"Performance tests: {result['passed']} passed, {result['failed']} failed"
        )
